...
extern uint8_t _binary_build_user_program2_elf_start[];
extern uint8_t _binary_build_user_program2_elf_end[];
...
void kernel_run_demo(void) {
    const uint8_t* image1 = _binary_build_user_program_elf_start;
    uint32_t size1 = (uint32_t)(_binary_build_user_program_elf_end -
                                _binary_build_user_program_elf_start);
    const uint8_t* image2 = _binary_build_user_program2_elf_start;
    uint32_t size2 = (uint32_t)(_binary_build_user_program2_elf_end -
                                _binary_build_user_program2_elf_start);

    uint32_t entry1 = 0;
    uint32_t entry2 = 0;

    kputs("Load two ELF user programs into memory partitions...\n");

    if (!load_user_elf_image(image1, size1, "user_program", &entry1) ||
        !load_user_elf_image(image2, size2, "user_program2", &entry2) ||
    ...
    pic_init();
    pit_init();
    kputs("Timer IRQ enabled. Round-robin scheduling starts...\n");
